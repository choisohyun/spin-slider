...
const useSwipe = ({ onSwipeLeft, onSwipeRight, minSwipeDistance = 50 }) => {
    ...
    const wasSwipedRef = react.useRef(false);
    /**
     * 시작점과 현재점의 차이를 계산합니다.
     */
    const getDelta = (x, y) => {
...
const SpinSliderPagination = ({ itemsLength, visibleCount, currentIndex, setCurrentIndex, maxVisible = 7, styles, accessibility, }) => {
    ...
    return (jsxRuntime.jsxs(SpinSliderPaginationWrapper, { role: "tablist", "aria-label": accessibility?.paginationLabel || '페이지네이션', children: [visiblePages[0] > 0 && (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx(PaginationDot, { "$active": false, "$styles": styles, onClick: () => goToPage(0), "aria-label": accessibility?.paginationLabel ? `${accessibility.paginationLabel} 첫 페이지로 이동` : '첫 페이지로 이동', role: "tab" }), visiblePages[0] > 1 && jsxRuntime.jsx("span", { children: "..." })] })), visiblePages.map(pageIndex => (jsxRuntime.jsx(PaginationDot, { "$active": pageIndex === currentPage, "$styles": styles, onClick: () => goToPage(pageIndex), "aria-label": accessibility?.paginationLabel
                    ? `${accessibility.paginationLabel} ${pageIndex + 1}페이지로 이동`
                    : `${pageIndex + 1}페이지로 이동`, role: "tab", "aria-selected": pageIndex === currentPage }, pageIndex))), visiblePages[visiblePages.length - 1] < totalPages - 1 && (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [visiblePages[visiblePages.length - 1] < totalPages - 2 && jsxRuntime.jsx("span", { children: "..." }), jsxRuntime.jsx(PaginationDot, { "$active": false, "$styles": styles, onClick: () => goToPage(totalPages - 1), "aria-label": accessibility?.paginationLabel
...
const SpinSliderItem = ({ item, index, width, sidePeek, currentIndex, totalItems, wasSwipedRef, onClick, renderItem, }) => {
    ...
    return (jsxRuntime.jsx(SpinSliderItemWrapper, { width: width, sidePeek: sidePeek, "aria-roledescription": "carousel item", "aria-label": `carousel item ${index + 1} of ${totalItems}`, "aria-current": currentIndex === index ? 'true' : 'false', onClick: handleClick, children: renderItem(item, index) }));
};
...
const SpinSlider = ({ items, renderItem, visibleCount = 1, sidePeek = 32, autoPlay = false, autoPlayInterval = 3000, infinite = false, showPagination = true, showNavigation = true, minSwipeDistance = 50, transitionDuration, initialIndex = 0, styles, callbacks, accessibility, className, }) => {
    ...
    // 계산된 값들
    const visibleAreaWidth = react.useMemo(() => viewportWidth - sidePeek * 2, [viewportWidth, sidePeek]);
    const itemWidth = react.useMemo(() => visibleAreaWidth / visibleCount, [visibleAreaWidth, visibleCount]);
    const translateX = react.useMemo(() => -(itemWidth * currentIndex) + sidePeek, [itemWidth, currentIndex, sidePeek]);
    const totalItems = items.length;
    const { startIndex, endIndex } = react.useMemo(() => getRenderRange(currentIndex, visibleCount, totalItems), [currentIndex, visibleCount, totalItems]);
    // 네비게이션 핸들러
    const goToPrevious = () => {
    ...
    return (jsxRuntime.jsxs(SpinSliderWrapper, { className: className, onTouchStart: handleTouchStart, onTouchEnd: handleTouchEnd, onMouseDown: handleMouseDown, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave, onKeyDown: handleKeyDown, tabIndex: isMouseDown ? -1 : 0, userSelect: isMouseDown ? 'none' : undefined, role: "region", "aria-roledescription": "carousel", "aria-label": accessibility?.label || '슬라이더', children: [showNavigation && (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx(SpinSliderButton, { "$position": "left", "$styles": styles, onClick: goToPrevious, "aria-label": accessibility?.previousButtonLabel || '이전 슬라이드', children: "\u2039" }), jsxRuntime.jsx(SpinSliderButton, { "$position": "right", "$styles": styles, onClick: goToNext, "aria-label": accessibility?.nextButtonLabel || '다음 슬라이드', children: "\u203A" })] })), jsxRuntime.jsxs(SpinSliderTrack, { "$translateX": translateX, "$transitionDuration": transitionDuration, children: [startIndex > 0 && jsxRuntime.jsx("li", { style: { width: startIndex * itemWidth, flexShrink: 0 } }), items.slice(startIndex, endIndex).map((item, i) => {
                        const index = startIndex + i;
                        return (jsxRuntime.jsx(SpinSliderItem, { item: item, index: index, width: itemWidth, sidePeek: sidePeek, currentIndex: currentIndex, totalItems: totalItems, wasSwipedRef: wasSwipedRef, onClick: handleItemClick, renderItem: renderItem }, index));
                    }), endIndex < totalItems && jsxRuntime.jsx("li", { style: { width: (totalItems - endIndex) * itemWidth, flexShrink: 0 } })] }), showPagination && (jsxRuntime.jsx(SpinSliderPagination, { itemsLength: items.length, visibleCount: visibleCount, currentIndex: currentIndex, setCurrentIndex: handleIndexChange, maxVisible: 10, styles: styles, accessibility: accessibility }))] }));
};

// 메인 컴포넌트

exports.SpinSlider = SpinSlider;
exports.SpinSliderItem = SpinSliderItem;
exports.SpinSliderPagination = SpinSliderPagination;
exports.default = SpinSlider;
exports.getCurrentPage = getCurrentPage;
exports.getNextIndex = getNextIndex;
exports.getPreviousIndex = getPreviousIndex;
exports.getRenderRange = getRenderRange;
